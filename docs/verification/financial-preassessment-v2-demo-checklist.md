...
背景：Q 指示演示交互统一「点按钮发起」（问答点提问才分析、预评估点生成报告才出匹配情况），预评估按钮移到文件选择器之后与问答对齐；资产 ID 对演示隐藏（BFF 新增受控样例端点 POST /api/controlled-sample/assess 自动解析白名单样例资产、POST /api/controlled-sample/query 供问答，前端不再暴露 asset_ids 输入）；模型本地/联网切换（本地默认 Ollama qwen3.5:9b / 联网 deepseek-4-flash，联网工具维修中 REFUSED 属预期）；顶栏用户头像（首字母大写）+ hover 登出；登录面板纳入 640px 居中；标题去 DEMO；报告精简（保留 匹配度/结果级别/缺失材料/引用/免责声明，去掉 查询主体/规则版本证据）。2026-08-27 起本地默认模型调整为 qwen3.6:27b-q4_K_M；2026-09-17 确认本地推理为 llama.cpp（llama-server，端点 127.0.0.1:18080、模型 qwen3.6:27b-q4_K_M，启用 --api-key 鉴权时 Key 经 RAG_LLM_LOCAL_API_KEY 注入），界面标签由「Ollama」更正为「llama」，新用户按 README「LLM 模型配置」用环境变量覆盖。2026-09-18 端到端实测：llama-server 实际模型 ID 为 qwen3.8-27b-local（请求中的 model 字段被忽略），本地默认模型/标签对齐为 qwen3.8-27b-local；RAG LLM 默认超时由 30s 放宽至 120s（推理模型含思考过程、耗时较长）；注入 Key 后受控问答 ANSWERED。2026-09-18 云端部署验证（106.53.200.115，Ubuntu 24.04，/opt/agent-workspace-tools，systemd agent-demo，部署 HEAD=0adcb44）：公网 /demo/ 200、alice 登录 200、预评估 MATCH 100、本地问答 REFUSED(llm_unavailable)（云端无本地模型，属预期）、bob 越权 DENIED 零召回；DeepSeek Key 走页面填写，详见 docs/deployment/cloud-deploy-2026-08-20.md 第 7 节。
